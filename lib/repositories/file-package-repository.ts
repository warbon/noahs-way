...
function normalizePackageRecord(
  ...
  const id =
    typeof record.id === "string" && record.id.trim()
      ? record.id.trim()
      : createPackageId(category, record.title, index)
...
async function getPackageCatalogStore() {
  try {
...
async function createPackageRecord(payload: CreatePackagePayload) {
  const catalog = await getPackageCatalogStore()
...
export const filePackageRepository: PackageRepository = {
